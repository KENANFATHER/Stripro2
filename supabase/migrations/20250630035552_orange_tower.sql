/*
  # Add Stripe Disconnect Logging Support

  1. Security Events Table Updates
    - Ensure security_events table supports Stripe disconnect events
    - Add indexes for better query performance

  2. User Metadata Updates
    - Support for disconnect timestamps and reasons in auth.users metadata

  3. Data Retention Policies
    - Set up policies for GDPR compliance
*/
...
GRANT EXECUTE ON FUNCTION request_gdpr_deletion(UUID, TEXT) TO authenticated;
...
COMMENT ON FUNCTION cleanup_old_security_events() IS 'Cleans up security events older than 2 years for GDPR compliance';
COMMENT ON FUNCTION request_gdpr_deletion(UUID, TEXT) IS 'Handles GDPR data deletion requests with proper logging';
